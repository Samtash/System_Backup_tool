...
```bash
git clone https://github.com/Samtash/System_Backup_tool.git
...
1. `create_backup_folder()` stamps a new version directory inside the destination.
2. `get_all_files()` walks the source tree with `os.walk` and collects every file path.
3. Each file gets its path rebuilt relative to the source root, parent folders are created on demand, then `copy2` moves it over.
4. After every file the progress callback fires and the UI redraws.
